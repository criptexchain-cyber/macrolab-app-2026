...
with st.sidebar:
    ...
    st.markdown("---")
    # AQUÍ ESTÁ EL SELECTOR LINEAL / CICLADO
    mode = st.radio("Estrategia", ["Lineal (Estable)", "Ciclado (ON/OFF)"])
    dias = st.slider("Días Gym", 1, 7, 4)
    ...
    if st.button("🚀 CALCULAR"):
        ...
        if "Lineal" in mode:
            st.session_state.m_off = base
        else:
            st.session_state.m_off = calc_off(base)
...
if st.session_state.ok:
    ...
    with t1:
        st.info(f"Rutina para {st.session_state.dias} días (Ejemplo):")
    ...
    with t4:
        ...
        d_gym = st.session_state.dias
        d_rest = 7 - d_gym
        
        for d, count in [(st.session_state.d_on, d_gym), (st.session_state.d_off, d_rest)]:
            for k, v in d.items():
                for x in v: lista[x['n']] += (x['g'] * count)
    ...
    with t5:
        ...
        # Enlaces seguros (sin f-strings largas)
